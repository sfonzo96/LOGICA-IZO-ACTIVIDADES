/*
	Consigna:
		...
		Elaborar programa para cobro de peaje:
			...
			- Se deben grabar también, la patente del vehículo, el tiempo y fecha del cobro y la categoría
*/
...
int main() {
	...
	while (!cargaEstaTerminada) {
		// Pide al usuario la categoría del vehículo
		...
		// Pide al usuario la patente del vehículo
		printf("Ingrese la pantente del vehículo, sin espacios:\n");
		...
		fprintf(historialTxtAppend, "%.2f,%s,%s,%s", totalACobrar, tipoVehiculo == 1? "Auto" : "Camión", patente, asctime(localtime(&payTime)));
